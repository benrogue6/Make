...
class lane_track: # camera class
  ...
  def image_callback(self, msg):
    ...
    # inserting the range for the grey color in hsv format
    lower_grey = numpy.array([ 0, 0, 0])
    upper_grey = numpy.array([100, 55, 190])
    ...
    cv2.imshow("grey_lane_mask",mask1)
    cv2.imshow("blue_cube_mask",mask2)
    cv2.imshow("camera_output", image)
    cv2.waitKey(3)

if __name__ == '__main__':
    rospy.init_node('lane_track') #initialize node
